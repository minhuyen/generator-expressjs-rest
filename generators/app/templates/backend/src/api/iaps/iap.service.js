...
function AppleVerifyErr(message, errors) {
...
class IapService extends Service {
  ...
  async createAndUpdateUserCredit(user, data) {
    ...
    if (response.status === 0) {
      ...
      throw new AppleVerifyErr("The receipt is not valid!!!", response);
    }
  }
      ...
      if (response["latest_receipt_info"]) {
        ...
        if (!iapCount) {
          ...
          if (latest_receipt_info["expires_date"]) {
            iapObj["end_date"] = parseMilisecond(
          ...
          iapCount.end_date = parseMilisecond(
            latest_receipt_info["expires_date_ms"]
          );
